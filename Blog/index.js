...
router.post("/post-blog", async (req, res) => {
  ...
  try {
    ...
    if (author.length < 4) {
      res.status(404).json({ message: "Author name is not valid" }).end();
    } //backend validation for author field
    if (title.length < 9) {
      res
        .status(404)
        .json({ message: "Title most be greater than 9 words" })
        .end();
    } //backend validation for Post title
    if (author_id.length < 10) {
      res.status(404).json({ message: "Author ID undefined" }).end();
    } //validating author_id same as User identity
    if (typeof post == "string") {
      // I checked if the typeof post is a string
      ...
      if (post_to_json.length > 3) {
        // if the post length is greater than 3 that means it's a valid post
...
router.post("/evaluate-post", async (req, res) => {
  ...
  try {
    ...
    if (Blog.length) {
      ...
      const newArray = await UpdateAnArray(
        new_item,
        post_to_json,
        Number(item_id)
      ); //Ref:(../Utilities)
      ...
      const update_post = await Export_BlogSchema.updateOne(
        { ide: blog_id },
        { post }
      );
      const update_percent = await Export_BlogSchema.updateOne(
        { ide: blog_id },
        { percentage }
      );
...
router.post("/update-post", async (req, res) => {
  ...
  try {
    ...
    if (typeof post == "string") {
      res.status(404).json({ message: "New Post is not a string" }).end();
    } //check the post type
    if (blog_id.length > 4) {
      res.status(404).json({ message: "New Post is not a string" }).end();
    } //Validate Blog identity
    const update = await Export_BlogSchema.updateOne(
      { ide: blog_id },
      { post }
    ); //Upadte new post
    if (update) {
